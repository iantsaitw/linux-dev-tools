...
# hwsim 通常位於 drivers/net/wireless/virtual/
...
# --- 第一階段：編譯 cfg80211 ---
echo -e "\e[1;34m📂 [1/4] Processing cfg80211: $WIRELESS_DIR\e[0m"
...
echo -e "\e[1;34m📂 [2/4] Processing mac80211: $MAC80211_DIR\e[0m"
...
# --- 第三階段：編譯 rtw89 (STA 實體網卡端) ---
echo -e "\e[1;34m📂 [3/4] Processing rtw89: $RTW89_DIR\e[0m"
...
# --- 第四階段：編譯 mac80211_hwsim (模擬器端) ---
echo -e "\e[1;34m📂 [4/4] Processing mac80211_hwsim: $HWSIM_DIR\e[0m"
# hwsim 通常沒有自定義 Makefile.local，我們直接調用核心內部的 M= 模式
[ "$DO_CLEAN" = true ] && make -C "$KERNEL_ROOT" M="$HWSIM_DIR" clean
make -C "$KERNEL_ROOT" M="$HWSIM_DIR" -j$(nproc) \
     KBUILD_EXTRA_SYMBOLS="$WIRELESS_DIR/Module.symvers $MAC80211_DIR/Module.symvers"
if [ $? -ne 0 ]; then echo -e "\e[1;31m❌ Error in hwsim\e[0m"; exit 1; fi
...
KO_FILES=(
    ...
    "$HWSIM_DIR/mac80211_hwsim.ko"
)

echo -e "\n========================================================"
echo -e "📊  STA (RTK) & SIMULATOR BUILD SUMMARY"
echo -e "========================================================"
...
for KO in "${KO_FILES[@]}"; do
    ...
    if [ -f "$KO" ]; then
        ...
        printf "\e[32m%-22s %-10s %-20s\e[0m\n" "$MOD_NAME" "Ready" "$BUILD_TIME"
    else
        printf "\e[31m%-22s %-10s %-20s\e[0m\n" "$MOD_NAME" "MISSING" "N/A"
    fi
done
echo -e "========================================================\n"